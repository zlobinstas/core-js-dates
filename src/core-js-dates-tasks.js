...
function getNextFriday(date) {
  const currentDayOfWeekIndex = date.getUTCDay();
  const daysUntilNextFriday =
    currentDayOfWeekIndex < 5
      ? 5 - currentDayOfWeekIndex
      : 5 + (7 - currentDayOfWeekIndex);
...
function formatDate(date) {
  ...
  return `${month}/${day}/${year}, ${hours}:${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')} ${period}`;
}
...
function getWeekNumberByDate(date) {
  ...
  const dayOffset = (10 - tempDate.getDay()) % 7;
  tempDate.setDate(tempDate.getDate() + dayOffset);
  return Math.ceil((date - tempDate) / (7 * 24 * 60 * 60 * 1000)) + 1;
}
